...
## ADR-085 — Extraction Updates Must Reach the Canonical Project Before Discovery Runs

**Status:** Accepted — MVP bug fix (`/generate`, `/visualize`)

**Decision:** Both routes that turn a raw brief into a `CanonicalProject` (Six-File Generation and Visual Architecture Map) built their canonical project by cloning `oxzire3dWebsiteFixture`, blanking every field to `"missing"`, and setting only `identity.name`/`identity.oneLiner` from the real project. `/generate` separately called `extractCanonicalUpdates()` on the brief, but only read its `updates[]` array for markdown text — the extracted values were never written back onto the canonical project's fields, so `analyzeDiscovery()` (completeness, project type, resolved fields, open questions) always ran against the blanked clone regardless of brief content. `/visualize` never called extraction at all. Net effect: the Visual Map rendered the same near-empty "submit a detailed brief to populate" diagram no matter what was typed.
