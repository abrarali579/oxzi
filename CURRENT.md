...
- **Step 10 — Agent Control Plane & Execution Passport** — Upgraded `executionPassportSchema` with scope (writable/readOnly/forbiddenFiles), limits (maxTokens/maxTimeMs), expiresAt, status (ACTIVE/EXPIRED/REVOKED). New `runtime.ts` with policy authorization check, cryptographic hash signing, `verifyPassportValidity()` returning `PassportVerificationResult`, `checkPassportScope()` for per-file enforcement, and `revokePassport()`. Updated agent-delivery dispatch to use new verification API. Tests covering issuance, rejection, expiry, tamper detection, scope checks, and revocation.
- **Step 11 — Approval-gated Agent Delivery** — Risk-based approval gates (low/medium auto, high/critical require `HumanApprovalRecord` with role check). `verifyApprovalGate()` enforces signature verification and role authority (high→admin+, critical→owner). Tests for pending approval, forged signatures, role insufficiency, and valid dispatch.
- **Step 12 — Internal UI + Persistence + API Layer** — Standardized `apiSuccess`/`apiError` response helpers, safe Content-Type guards in form handlers, Prisma indexes, Zod DB validation wrappers.
- **Step 13 — Auth, Billing, Teams & Multiplayer** — Extended roles to include `viewer`, added `requireAdminOrOwner()` for high-risk approvals, created `lib/billing/quota.ts` with deterministic `checkQuota()` and `consumeQuota()` for organization-level token/passport limits.
- **Step 14 — Connected Execution Runtime Foundation** — Implemented `src/domain/execution/runtime.ts` bridging Agent Control Plane, rendered Prompt Programs, and connected agent execution. Includes `ExecutionPassport` validation, agent capability role-matching (Architect/Executor/Reviewer), provider-neutral dispatch through `ProviderAdapter` interface, artifact-first output capture with structured contract parsing, trace/span generation via `src/domain/observability/`, and typed `ExecutionReport` generation. A `MockProviderAdapter` provides deterministic simulated execution for testing without real API calls. Schemas added: `ExecutionReportSchema`, `ProviderAdapterConfigSchema`, `AgentCapabilityMappingSchema`, `AgentRoleSchema`. 18 tests covering happy path, timeout/parse failure/empty output, passport validation, capability matching, and schema validation. No canonical mutation — execution is strictly operational.
